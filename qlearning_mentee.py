...
def build_dqn(num_actions, action_repeat):
    ...
    dqn = models.build_mentor_model_dqn(inputs, num_actions)
    return inputs, dqn.output, dqn
...
def actor_learner_thread(thread_id, env, session, graph_ops, num_actions,
    ...
    while T < TMAX:
        ...
        while True:
            ...
            # TODO: here also get the 'correct' outputs according to the mentor
            ...
            if T % I_target == 0:
                ...
                graph_ops['target_q_network'].save("qlearning_dqn_target.h5")
            ...
            if t % I_AsyncUpdate == 0 or terminal:
                if s_batch:
                    ...
                    # TODO: update gradients given mentor outputs and mentee error
            ...
            if terminal:
                ...
                if ep_reward > best_reward:
                    graph_ops['shared_q_network'].save("qlearning_dqn_shared.h5")
...
def build_graph(num_actions):
    ...
    inputs, outputs, shared_q_network = build_dqn(num_actions=num_actions,
                             action_repeat=action_repeat)
    s = inputs
    q_values = outputs
    ...
    inputs, outputs, target_q_network = build_dqn(num_actions=num_actions,
                                     action_repeat=action_repeat)

    st = inputs
    target_q_values = outputs
    ...
    cost = tf.sqrt(tf.reduce_mean(tf.square(tf.sub(action_q_values, y))))
